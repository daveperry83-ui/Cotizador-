...
T = {
    "es": {
        ...
        "browser_mode_note": "🔒 Ejecutándose en el navegador (sin servidor): sube tu archivo, se procesa localmente y nada se envía a internet.",
    ...
    "en": {
        ...
        "browser_mode_note": "🔒 Running in your browser (no server): upload your file, it's processed locally and nothing is sent anywhere.",
...
with st.sidebar:
    ...
    _IN_BROWSER = (sys.platform == "emscripten")  # True dentro de stlite/Pyodide (WebAssembly)

    if _IN_BROWSER:
        mode = "upload"
        st.caption(t["browser_mode_note"])
    else:
        mode = st.radio(
    ...
    if mode == "local":
        ...
        if st.button(t["local_load"], width="stretch"):
    ...
    if st.session_state.local_path:
        ...
        if st.button(t["local_save_btn"], type="primary", width="stretch"):
        ...
        if q.strip():
            ...
            if not len(res):
                ...
                with pc2:
                    st.write("")
                    st.write("")
                    if st.button(t["use_in_quote"], type="primary", width="stretch"):
...
with tab_quote:
    ...
    with right:
        ...
        st.markdown("---")
        if st.button(t["add_quote"], type="primary", width="stretch"):
...
with tab_batch:
    ...
    if not cart:
        ...
        st.dataframe(internal_view, width="stretch", hide_index=True)
        ...
        try:
            ...
            e2.download_button(
                ...
                help=t["export_client_help"], width="stretch",
            )
        except Exception as exc:  # noqa: BLE001
